...
/*29updated*/	else if ($getName=='staffCertificate') { $dbName= htmlspecialchars($_GET['dbName']);  $pKV= htmlspecialchars($_GET['pKV']);  $data=array(); $D1=array(); $D2=array();	$totalCr=0; 
			try { $Curdate = new DateTime("now", new DateTimeZone('America/Phoenix')); $Curdate=$Curdate->format('Y-m-d'); $conn= mysqli_connect ($servername, $username, $r_P, $dbName);  
				$query="SELECT   MAX(certified.CertExDate) as maxDate, certified.CrsCode,  certified.Certtaken from Employee INNER JOIN certified ON certified.EmpId=Employee.EmpId WHERE certified.EmpId='". $pKV."' AND certified.CertExDate IS NOT NULL GROUP BY certified.CrsCode";  				
				$result= mysqli_query($conn, $query); if(mysqli_num_rows($result)>0) { while($row=mysqli_fetch_array($result)){ $D1[]=$row; if ($row['CrsCode']=='DD-Caregiver' || $row['CrsCode']=='DD-Operator') { $maxD=$row['maxDate']; $maxDY= date_create($maxD); date_add($maxDY, date_interval_create_from_date_string("-364 days"));  $maxDY=$maxDY->format('Y-m-d');}  } }  }  catch(Exception $e) { $D1[]='What is Going on here!'; } 
			try {  
				$query="SELECT  trainningcertificates.CrHrs, trainningcertificates.certification, certified.CertExDate, certified.CrsCode, certified.Certtaken, employee.hiredD from Employee INNER JOIN certified ON certified.EmpId=Employee.EmpId JOIN trainningcertificates ON trainningcertificates.CrsCode=certified.CrsCode WHERE certified.EmpId='". $pKV."' AND trainningcertificates.CrHrs>0 AND (certified.Certtaken<'$maxD' AND certified.Certtaken> '$maxDY') ORDER BY certified.Certtaken DESC";  
